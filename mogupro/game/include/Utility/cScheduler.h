...
public:
	...
	void applyLimitUpdate( float updateSeconds, std::function<void(float)> method, std::function<void(float)> finalizeMethod = nullptr );
	void update( float delta );
private:
	hardptr<Node::node> mRoot;
	std::set<hardptr<int>> mHandles;
	Utility::cIdentifier mIdGen;
	struct LimitUpdateMethod { std::function<void(float)> update, finalize; };
