...
void Game::ProductionCamera::cProductionCamera::shakeCamera(const float & scatter, const float & seconds)
{
	this->scatter = scatter;
	this->seconds = seconds;
}

void Game::ProductionCamera::cProductionCamera::setup() {
...
//慣性つきカメラ移動
void Game::ProductionCamera::cProductionCamera::MovingCamera()
{
	buf_pos = ci::vec3((*reference_pos)->x, (*reference_pos)->y, (*reference_pos)->z) - pos;
	buf_pos *= 0.3f;
	pos += buf_pos;
}
void Game::ProductionCamera::cProductionCamera::ScatterCamera()
{
	if (seconds < 0)return;
...
void Game::ProductionCamera::cProductionCamera::update(const float& delta_time) {
	//ブレるカメラの秒数をデルタタイムで引く
	seconds -= delta_time;
...
void Game::ProductionCamera::cProductionCamera::draw()
{
