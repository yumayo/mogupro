...
boost::optional<Packet::Request::cReqInRoomWatching> cRequestManager::getReqInRoomWatching( )
{
    if ( mReqInRoomWatching.empty( ) )
    {
        auto it = mReqInRoomWatchingSequenceIds.begin( );
		while ( it != mReqInRoomWatchingSequenceIds.end( ) ) 
		{
			if ( it->second < cinder::app::getElapsedSeconds( ) - RELIABLE_HOLD_SECOND )
			{
				mReqInRoomWatchingSequenceIds.erase( it++ );
			}
			else ++it;
		}
        return boost::none;
    ...
    {
        auto top = mReqInRoomWatching.top( );
        mReqInRoomWatching.pop( );
        return top;
    }
}
void cRequestManager::ungetReqInRoomWatching( Packet::Request::cReqInRoomWatching&& data )
{
    mReqInRoomWatching.push( std::move( data ) );
}
bool cRequestManager::isNewReqInRoomWatching( Packet::PacketHeader const& header )
{
	if ( ( header.mState & Packet::PacketHeader::RELIABLE ) != Packet::PacketHeader::RELIABLE ) return true;
    auto status = mReqInRoomWatchingSequenceIds.insert( std::make_pair( header.mSequenceId, cinder::app::getElapsedSeconds( ) ) );
    status.first->second = cinder::app::getElapsedSeconds( );
	return status.second;
}
...
boost::optional<Packet::Request::cReqFragmentGem> cRequestManager::getReqFragmentGem( )
{
    if ( mReqFragmentGem.empty( ) )
    {
        auto it = mReqFragmentGemSequenceIds.begin( );
		while ( it != mReqFragmentGemSequenceIds.end( ) ) 
		{
			if ( it->second < cinder::app::getElapsedSeconds( ) - RELIABLE_HOLD_SECOND )
			{
				mReqFragmentGemSequenceIds.erase( it++ );
			}
			else ++it;
		}
        return boost::none;
    ...
    {
        auto top = mReqFragmentGem.top( );
        mReqFragmentGem.pop( );
        return top;
    }
}
void cRequestManager::ungetReqFragmentGem( Packet::Request::cReqFragmentGem&& data )
{
    mReqFragmentGem.push( std::move( data ) );
}
bool cRequestManager::isNewReqFragmentGem( Packet::PacketHeader const& header )
{
	if ( ( header.mState & Packet::PacketHeader::RELIABLE ) != Packet::PacketHeader::RELIABLE ) return true;
    auto status = mReqFragmentGemSequenceIds.insert( std::make_pair( header.mSequenceId, cinder::app::getElapsedSeconds( ) ) );
    status.first->second = cinder::app::getElapsedSeconds( );
	return status.second;
}
// P=====END=====P
