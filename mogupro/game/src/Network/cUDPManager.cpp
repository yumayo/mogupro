...
void cUDPManager::onReceive( cPacketChunk const & packetChunk )
{
    ...
    case Network::Packet::PacketId::RES_PLAYER:
    {
        ...
        data.onReceive( networkHandle, bufferSize, bufferData );
        cResponseManager::getInstance( )->ungetResPlayer( std::move( data ) );
        break;
    }
    ...
    case Network::Packet::PacketId::RES_BREAK_BLOCKS:
    {
        Packet::Response::cResBreakBlocks data;
        data.onReceive( networkHandle, bufferSize, bufferData );
        cResponseManager::getInstance( )->ungetResBreakBlocks( std::move( data ) );
        break;
    }
