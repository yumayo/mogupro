...
namespace Scene
{
	namespace Member
	{
		...
		void cMatching::updateBoxFunc()
		{
			...
			{
				while (auto resCheckBeginGame = cUDPServerManager::getInstance( )->getUDPManager( )->ResCheckBeginGame.get())
				{
		...
		void cMatching::makeRoom()
		{
			...
			{
				while (auto resMakeRoom = cUDPServerManager::getInstance( )->getUDPManager( )->ResMakeRoom.get())
				{
					continue;
				}
			...
			{
				while (auto resInRoom = cUDPServerManager::getInstance( )->getUDPManager( )->ResInRoom.get())
				{
		...
		void cMatching::inRoom()
		{
			...
			//Teamに入れたかどうか
			while (auto resWantTeamIn = cUDPServerManager::getInstance( )->getUDPManager( )->ResWantTeamIn.get())
			{
			...
			int count = 0;
			while (auto eveTeamMember = cUDPServerManager::getInstance( )->getUDPManager( )->EveTeamMember.get())
			{
